...
TARGET_DIR="$1"
SOURCE_DIR="$(cd "$(dirname "$0")/.." && pwd)"  # Gets /dotnet directory by going up from scripts
...
build_and_publish "$SOURCE_DIR/src/test-control-apps/TestControl.Cli/TestControl.Cli.csproj" "$TARGET_DIR"
...
for host_project in "$SOURCE_DIR"/src/test-subjects/*/*.WebApi; do
