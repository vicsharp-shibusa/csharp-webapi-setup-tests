...
public class ReportService : BaseService, IReportService
{
    ...
    public Task<IEnumerable<UserTransaction>> GetTransactionsForUserAsync(Guid userId, DateTime? start = null, DateTime? finish = null, string status = null)
    {
        // Set default date range if not provided
        start ??= DateTime.Now.AddMinutes(-15);
        finish ??= DateTime.Now.AddSeconds(-5);
        ...
        return _userTransactionRepository.GetTransactionsForUserAsync(userId, start.Value, finish.Value, status);
    }
}
