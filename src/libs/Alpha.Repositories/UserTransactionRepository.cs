...
public class UserTransactionRepository : IUserTransactionRepository
{
    ...
    public async Task<UserTransaction> GetByIdAsync(Guid transactionId)
    {
        var dao = await _queryConnection.QuerySingleOrDefaultAsync<UserTransactionDao>(
            _sqlProvider.GetSql(SqlKeys.GetTransactionById), new { TransactionId = transactionId });
        ...
        if (dao != null)
        {
            user = await _userRepository.GetByIdAsync(dao.UserId);
            org = await _organizationRepository.GetByIdAsync(dao.OrganizationId);
        }

        return dao?.ToDto(org, user);
    }

    public async Task<IEnumerable<UserTransaction>> GetForOrganizationAsync(Guid organizationId)
    {
        ...
        var daos = (await _queryConnection.QueryAsync<UserTransactionDao>(
            _sqlProvider.GetSql(SqlKeys.GetTransactionsForOrganization), new
            {
                organizationId
            })).ToArray();
    ...
    public async Task UpsertAsync(UserTransaction transaction, Guid operationId)
    {
        await _commandConnection.ExecuteAsync(_sqlProvider.GetSql(SqlKeys.UpsertTransaction),
            new UserTransactionDao(transaction, transaction.User.UserId));
    }

    public async Task<IEnumerable<UserTransaction>> GetTransactionsForOrganizationAsync(Guid orgId, DateTimeOffset start, DateTimeOffset finish, string status = null)
    {
        ...
        var users = await _userRepository.GetForustomerOrganizationAsync(orgId);
        ...
        var transactionDaos = await _queryConnection.QueryAsync<UserTransactionDao>(sql, parameters);
    ...
    public async Task<IEnumerable<UserTransaction>> GetTransactionsForUserAsync(Guid userId, DateTimeOffset start, DateTimeOffset finish, string status = null)
    {
